...
#if defined(NDEBUG)
# error "Turk cannot be compiled without assertions."
#endif
...
unsigned int nModifierInterval = 18 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 49; // 69 in total
...
const string strMessageMagic = "Turk Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nHeight, int64_t nFees)
{
    ...
    else if(pindexBest->nHeight < 999)
    {
        nSubsidy = 69 * COIN;
    }
    ...
    else if(pindexBest->nHeight < 2499)
    {
        nSubsidy = 69 * COIN;
    }
    ...
    else if(pindexBest->nHeight < 6969)
    {
        nSubsidy = 6969 * COIN;
    }
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8); // 69% Per Annual Year
    ...
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8); // 69% Per Annual Year
    }
    else if(pindexBest->nHeight < 2000)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10; // 690% Per Annual Year
    }
    else if(pindexBest->nHeight < 2499)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8); // 69% Per Annual Year
    ...
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) / 10; // 69% Per Annual Year
    }
    else if(pindexBest->nHeight < 6000)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8); // 69% Per Annual Year
    ...
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 101; // 6969% Per Annual Year
    }
